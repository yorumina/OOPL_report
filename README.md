...
本專案復刻《明日方舟》的核心塔防玩法。玩家需要在格子地圖上部署幹員，依照地形限制選擇地面或高台位置，消耗部署費用防守敵人波次，阻止敵人抵達目標點。
...
| 畫面 | 說明 |
|------|------|
| <img src="Operation 1-1.png" width="520"> | `Operation 1-1` 關卡背景 |
| <img src="Operation 1-2.png" width="520"> | `Operation 1-2` 關卡背景 |
| <img src="Loading_Operation 1-1.png" width="520"> | `Operation 1-1` 關卡載入畫面 |
| <img src="Finish_Operation 1-1.png" width="520"> | `Operation 1-1` 關卡完成畫面 |
...
本專案有使用 AI/AI Agent 輔助，但主要的程式架構、模組切分、遊戲流程設計與核心邏輯都是由我自行規劃與完成。包含 `App` 主流程、`GameLogic`、`EnemySystem`、`OperatorSystem`、`StageLoader`、`Renderer` 與`ArknightBuilder` 的分工，都是依照專案需求設計出來的。

1. 協助整理報告文字，將已完成的程式架構與功能轉成較清楚的說明。
2. 協助檢查程式碼與文件中是否有格式不一致、說明不清楚或容易遺漏的地方。
3. 協助補充部分實作細節的描述。
4. 協助調整 Markdown 格式。
5. 抓bug

因此，本專案的設計方向與主要程式實作是由我完成，AI 的角色比較像輔助工具，負責協助整理細節、檢查文件與改善表達方式。
...
我在這個專案中學到最多的是模組化與資料驅動的重要性。另外製作 `ArknightBuilder` 也讓關卡調整從手動改檔案變得更可控。最後完成兩個可銜接的關卡時，能看到資料、邏輯與畫面一起運作，這是這份專題最有成就感的地方。
