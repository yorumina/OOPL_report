...
| <img src="data/levels/Operation 1-1/Loading_Operation 1-1.png" width="520"> | 關卡載入畫面 |
| <img src="data/levels/Operation 1-1/Finish_Operation 1-1.png" width="520"> | 關卡完成畫面 |
...
3. `App::ResetDemo()` 初始化 DP、LP、波次、敵人、幹員與動畫狀態。
4. `UpdateGame()` 依照時間推進 DP、敵人生成、敵人移動、幹員攻擊與勝敗判斷。
5. `AppRenderer::DrawScene()` 依照遊戲狀態繪製背景、敵人、幹員、HUD、部署預覽與結算畫面。
...
| 資料驅動設計 | 關卡、敵人數值、幹員數值、動畫路徑與 UI 資源可由 `data/` 修改 |
| APNG / WebM 動畫 | 幹員與敵人動畫使用多方向素材，支援行走、攻擊、技能與死亡狀態 |
| FFmpeg 輔助流程 | 產生翻轉動畫與動畫快取，降低素材處理成本 |
| CLI 工具 | `ArknightBuilder` 可建立、修改、驗證、模擬與校準關卡 |
| Debug mode | `M` 顯示地圖模型，`Z` 切換作弊/debug 模式，`R` 快速重置 |
...
| Windows 中文路徑與建置容易出錯 | 提供 `build_win.bat`，將專案同步到英文路徑建置，並在 MSVC 使用 `/utf-8` |
| 部署流程需要同時處理拖曳、合法格、方向與攻擊範圍 | 將部署拆成「拖曳選格」與「方向選擇」兩階段，並用預覽格與攻擊範圍回饋玩家 |
| 敵人與幹員動畫狀態容易互相干擾 | 將動畫 clip 與 active instance 分開管理，依照 idle、move、attack、skill、die 狀態切換 |
| 敵人波次與路線手動測試成本高 | 在 `ArknightBuilder` 加入 `validate` 與 `simulate`，可先檢查 JSON 與時間軸再進遊戲 |
